...
class PcscDriverAdaptor {
 public:
  ...
  // Returns the driver file name - typically of the "foobar.so" form.
  //
  // It's expected that the driver returns the same string as the one specified
  // in its Info.plist config file.
  virtual const std::string& GetDriverFileName() const = 0;

  // Returns to the driver's exported functions table.
  virtual const std::vector<const FunctionNameAndAddress>&
  GetFunctionPointersTable() const = 0;
};
