...
CcidPcscDriverAdaptor::CcidPcscDriverAdaptor()
    : kFileName_(
          "libccid.so"),  // Should match the value passed via "--target" to
                          // create_Info_plist.pl at ../build/Makefile.
      kFunctionPointers_({
...
const std::string& CcidPcscDriverAdaptor::GetDriverFileName() const {
  return kFileName_;
}

const std::vector<const CcidPcscDriverAdaptor::FunctionNameAndAddress>&
