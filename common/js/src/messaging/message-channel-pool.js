...
GSC.MessageChannelPool = class {
  ...
  addChannel(messagingOrigin, messageChannel) {
    ...
    messageChannel.addOnDisposeCallback(this.handleChannelDisposed_.bind(
        this, messagingOrigin, messageChannel));
  ...
  fireOnUpdateListeners_() {
    ...
    for (let listener of this.onUpdateListeners_) {
      try {
        ...
        // Rethrow the listener's exception asynchronously, in order to not
