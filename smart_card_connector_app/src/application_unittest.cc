...
TEST_F(SmartCardConnectorApplicationTest, StressReaderAddingRemoving) {
  const int kIterationCount = 30;
  // The constant is arbitrary, but to exercise more interesting scenarios it
  // should be bigger than 1000 ms, so that the event loop in libusb_hotplug.c a
  // has chance to notice the device change.
  const int kMaximumDelayMs = 2000;

  // Arrange:

  // Use a random engine with a constant (default) seed to keep the test
  // reproducible.
  std::mt19937 rng;
  std::uniform_int_distribution<> sleep_dist(0, kMaximumDelayMs);
  ...
  for (int i = 0; i < kIterationCount; ++i) {
    ...
    std::vector<TestingSmartCardSimulation::Device> devices;
    // Make the device repeatedly appear and disappear.
    if (i % 2 == 0) {
      TestingSmartCardSimulation::Device device;
      device.id = 123;
      device.type =
